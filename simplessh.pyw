...
class MainApplication(Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        connection_list_window.grid(column=0, row=1, columnspan=2, sticky=N+S, padx=5)
        add_profile_input = Entry()
        add_profile_input.grid(column=0, row=2, columnspan=2, sticky=N, pady=(0,5))
        add_profile = Button(text='Add', command=add_ssh_profile)
        add_profile.grid(column=0, row=3, sticky=W, padx=(5,0))
        delete_profile = Button(text='Delete', command=delete_ssh_profile)
        delete_profile.grid(column=1, row=3, sticky=E, padx=(0,5))
        ...
        port_input = Entry()
        port_input.grid(column=3, row=1, sticky=N, pady=20, padx=(0,5))
        username_tag = Label(text='Username(optional):')
        username_tag.grid(column=2, row=1, sticky=N+W, pady=50)
        username_input = Entry()
        username_input.grid(column=2, row=1, sticky=N, pady=70)
        password_tag = Label(text='Password(optional):')
        password_tag.grid(column=3, row=1, sticky=N+W, pady=50)
        password_input = Entry(show='*')
        password_input.grid(column=3, row=1, sticky=N, pady=70, padx=(0,5))
        pem_tag = Label(text='.PEM key(optional)')
        pem_tag.grid(column=2, row=1, sticky=N+W, pady=100)
        pem_input = Entry()
        pem_input.grid(column=2, row=1, sticky=N+W, pady=120, padx=(0,5))
        pick_pem = Button(text='...', command=select_pem_key)
        pick_pem.grid(column=3, row=1, sticky=N+W, pady=112)
        desc_tag = Label(text='Description:')
        desc_tag.grid(column=2, row=1, sticky=S+W, pady=70)
        description_window = Text(width=40, height=5)
        description_window.grid(column=2, row=1, columnspan=3, sticky=S+W, rowspan=2, pady=(0,5))
        
        #connect/advanced
        connect_to = Button(text='Connect', command=connect_shell)
        connect_to.grid(column=3, row=3, sticky=E, padx=(15))
        ...
        save_conn.grid(column=3, row=3, sticky=W, padx=(0,5))
